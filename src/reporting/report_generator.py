...
class ReportGenerator:
    ...
    def _setup_event_handlers(self) -> None:
        ...
        try:
            ...
            self.event_bus.subscribe(
                "data_loaded",  # Use lowercase to match your event system
            ...
            self.event_bus.subscribe(
                "report_generation_requested",  # Use lowercase to match your event system
            ...
            self.event_bus.subscribe(
                "analysis_completed",  # Use lowercase to match your event system
                self._handle_analysis_completed,
                priority=5,
                handler_id=f"{self.component_id}_analysis_completed"
            )
            
            self.logger.info("Report generator event handlers registered successfully")
            print("DEBUG: Enhanced report generator event handlers registered")
    ...
    def _handle_data_loaded(self, event) -> None:
        ...
        print("DEBUG: Enhanced report generator received DATA_LOADED event!")
        self.logger.info("Enhanced report generator received DATA_LOADED event")
        ...
        try:
            ...
            print(f"DEBUG: Event data keys: {list(event.data.keys()) if hasattr(event, 'data') and event.data else 'No data'}")
            print(f"DEBUG: Dataset attributes: {dir(dataset) if dataset else 'None'}")
            ...
            if dataset is not None:
                ...
                if hasattr(dataset, 'data'):
                    dataframe = dataset.data  # SoldierDataset.data
                    print(f"DEBUG: Found dataset.data")
                elif hasattr(dataset, 'dataframe'):
                    dataframe = dataset.dataframe  # Alternative attribute name
                    print(f"DEBUG: Found dataset.dataframe")
                    ...
                    if isinstance(dataset, pd.DataFrame):
                        dataframe = dataset
                        print(f"DEBUG: Dataset is already a DataFrame")
                    else:
                        # Try to find any attribute that's a DataFrame
                        for attr_name in dir(dataset):
                            if not attr_name.startswith('_'):
                                attr_value = getattr(dataset, attr_name)
                                if isinstance(attr_value, pd.DataFrame):
                                    dataframe = attr_value
                                    print(f"DEBUG: Found DataFrame in attribute: {attr_name}")
                                    break
                ...
                if dataframe is not None:
                    ...
                    if len(self.data) > 0:
                        ...
                        self._publish_status(f"Enhanced report generator ready with {len(self.data)} data records")
                    else:
                        self.logger.warning("DataFrame is empty")
                        print("DEBUG: DataFrame is empty")
                        self._publish_status("DataFrame is empty", "warning")
                else:
                    self.logger.warning("Could not find DataFrame in SoldierDataset")
                    print("DEBUG: Could not find DataFrame in SoldierDataset")
                    self._publish_status("Could not find DataFrame in SoldierDataset", "warning")
            else:
                self.logger.warning("No valid dataset found in data loaded event")
                print("DEBUG: No valid dataset found in data loaded event")
            ...
            print(f"DEBUG: Error handling data loaded event: {e}")
            import traceback
